...
class drone(Mobility):

    vehCmds = None

    def __init__(self, drones=None, duration=10, **kwargs):
        Mobility.thread_ = thread(name='drone', target=self.configureApp,
                                  args=(drones, duration), kwargs=dict(kwargs,))
        Mobility.thread_.daemon = True
        Mobility.thread_._keep_alive = True
        Mobility.thread_.start()

    def configureApp(self, drones, duration):
        ...
        parser.add_argument('--num_drones',         default=len(drones),type=int,           help='Number of drones (default: 3)', metavar='')
        ...
        parser.add_argument('--duration_sec',       default=duration,   type=int,           help='Duration of the simulation in seconds (default: 5)', metavar='')
        ...
        for i in range(0, int(ARGS.duration_sec*env.SIM_FREQ), AGGR_PHY_STEPS):
            ...
            #    env.render()
                for i in range(ARGS.num_drones):
                    posX = '{:06.2f}'.format(env.pos[i,0])
                    posY = '{:06.2f}'.format(env.pos[i, 1])
                    posZ = '{:06.2f}'.format(env.pos[i, 2])
                    drones[i].position = posX, posY, posZ
                    drones[i].set_pos_wmediumd(drones[i].position)
